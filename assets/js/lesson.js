...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const pdfViewer = document.getElementById('lesson-pdf-viewer');
    ...
    async function renderPDF(url) {
        // Wait for pdfjsLib to be loaded (since it's a module, it might load asynchronously)
        let retries = 0;
        ...
        for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
            ...
            // Adjust scale based on devicePixelRatio for sharper rendering
            const baseScale = 1.5;
    ...
    async function openPreviewModal(url, title) {
